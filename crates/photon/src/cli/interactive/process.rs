...
pub async fn guided_process(config: &Config) -> anyhow::Result<()> {
    ...
    loop {
        ...
        let api_key = llm_selection.as_ref().and_then(|s| s.api_key.clone());
...
fn prompt_output_path(
    ...
    loop {
        let Some(raw) = super::handle_interrupt(
            Input::<String>::with_theme(theme)
        ...
        if let Some(parent) = path.parent() {
            if !parent.as_os_str().is_empty() && !parent.exists() {
                ...
                eprintln!(
                    "  {}",
                    warn.apply_to(format!("Directory does not exist: {}", parent.display()))
                );
                continue;
            }
        }
